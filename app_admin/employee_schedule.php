...
<body data-layout="detached">
  ...
  <main id="main" class="main">
    ...
    height: 500px;">
          <div class="m-2">
            <button type="button" id="add_new" class="btn btn-outline-dark btn-rounded " data-bs-toggle="modal" data-bs-target="#add_schedule">
              <i class="bi bi-plus-circle"></i> Add
            </button>
            <button class="btn btn-outline-primary btn-rounded alignToTitle" id="bulk_insert_btn" data-bs-toggle="modal" data-bs-target="#upload_csv_finance"><i class="bi bi-arrow-bar-up bulk"></i> Bulk Add</button>
          </div>
  ...
  <div class="modal fade" id="upload_csv_finance" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
          ...
          <div class="modal-header">
            <h5 class="modal-title" id="staticBackdropLabel">Bulk Insert</h5>
...
<a id="template" href="<?php echo BASE_URL; ?>global_process/download.php?path=<?php echo EMP_SCHED_LIST; ?>">
  ...
  <div class="modal fade" id="row_modal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="staticBackdropLabel" style="color:red;">Invalid CSV<br>Please Enter New CSV</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
...
<script>
  $('.dropify').dropify({
    messages: {
        'default': 'Drag and drop a your schedule .CSV  Here',
      ...
      success: function(response) {
        ...
        if (response.error == true) {
          Toast.fire({
            ...
            icon: 'error',
            title: 'Invalid CSV',
            text: 'Please Enter a Valid CSV',
            timer: 1500
          })

          var html_code = "<table class='table table-striped'>";
          html_code += "<thead>";
          html_code += "<tr><th>Row Number</th>";
          html_code += "<th>employee id</th>";
          html_code += " <th>day</th>";
          html_code += "<th>start_time</th>";
          html_code += "<th>end_time</th>";
          html_code += " <th>section</th>";
          html_code += "<th>room</th>";
          html_code += "<th>subject_name</th>";
          html_code += "<th>subject_code</th>";
          html_code += "<th>Remarks</th></tr></thead>";
  ...
  (function() {
    ...
    $("#schedule_frm").on('submit', function(e) {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        var subject_code = rowTr.getElementsByClassName('subject_name')[0].value;
        if (subject_code) {
      ...
      $.ajax({
        ...
        beforeSend:function(){
      $('#schedule_frm :submit').html( ' <span class="spinner-border spinner-border-sm"></span>Loading..'); 
      $("#schedule_frm :input").prop( "disabled", true );
      $("#schedule_frm :button").prop( "disabled", true );
        },
  
  
        complete:function(){
      $('#schedule_frm :submit').html( 'Submit'); 
      $("#schedule_frm :input").prop( "disabled", false );
      $("#schedule_frm :button").prop( "disabled", false );
        },
        success: function(response) {
          $('tr').css("border", "none");
